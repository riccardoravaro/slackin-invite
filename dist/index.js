...
function slackin(_ref) {
  ...
  var serverWeb3 = new _web2.default(new _web2.default.providers.HttpProvider('https://mainnet.infura.io'));
  ...
  app.post('/invite', (0, _bodyParser.json)(), function (req, res, next) {
    ...
    if (signature) {
      ...
      var msgParams = { data: msg, sig: signature };
      ...
      TokenContract.balanceOf(recovered, function (error, result) {
        if (!error) {
          if (result.gt(0.01)) {
            ...
            return res.status(400).json({ msg: 'ERROR The account you provided does not hold GNO Tokens' });
          }
        } else {
          return res.status(400).json({ msg: 'ERROR could not check GNO Balance ' + error });
        }
      });
    } else {
